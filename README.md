...
jobs:
  sign-commit:
    ...
    steps:
      ...
      - name: Import GPG key
        id: gpg-import
        uses: coatl-dev/actions/gpg-import@v7.0.9
        with:
...
jobs:
  pip-compile:
    ...
    steps:
      ...
      - name: pip-compile-27
        uses: coatl-dev/actions/pip-compile-upgrade@v7.0.9
        with:
      ...
      - name: Detect changes
        id: git-diff
        uses: coatl-dev/actions/simple-git-diff@v7.0.9
        with:
          path: "${{ env.REQUIREMENTS_PATH }}"
...
```yml
      - name: Create Pull Request
        uses: coatl-dev/actions/pr-create@v7.0.9
        with:
...
```yml
    - name: Upload Python package to PyPI
      uses: coatl-dev/actions/pypi-upload@v7.0.9
      with:
        python-version: '2.7'
        check: 'false'
...
```yml
    - name: Set up Jython
      uses: coatl-dev/actions/setup-jython@v7.0.9
      with:
  ...
  pip-compile:
    ...
    steps:
      ...
      - name: pip-compile-312
        uses: coatl-dev/actions/uv-pip-compile-upgrade@v7.0.9
        with:
          path: "${{ env.REQUIREMENTS_PATH }}"
          python-version: '3.12'
