...
jobs:
  sign-commit:
    ...
    steps:
      ...
      - name: Import GPG key
        id: gpg-import
        uses: coatl-dev/actions/gpg-import@v0.9.3
        with:
...
jobs:
  pip-compile:
    ...
    steps:
      ...
      - name: pip-compile-27
        uses: coatl-dev/actions/pip-compile@v0.9.3
        with:
      ...
      - name: Detect changes
        id: git-diff
        uses: coatl-dev/actions/simple-git-diff@v0.9.3
        with:
          path: "${{ env.REQUIREMENTS_PATH }}"
...
```yml
      - name: Create Pull Request
        uses: coatl-dev/actions/pr-create@v0.9.3
        with:
  ...
  pre-commit-autoupdate:
    ...
    steps:
      ...
      - name: Update pre-commit hooks
        uses: coatl-dev/actions/pre-commit-autoupdate@v0.9.3
        with:
          gh-token: ${{ secrets.GH_TOKEN }}
  ...
  pypi-publish:
    ...
    steps:
      ...
      - name: python2-pypi-upload
        uses: coatl-dev/actions/pypi-upload@v0.9.3
        with:
