...
jobs:
  sign-commit:
    ...
    steps:
      ...
      - name: Import GPG key
        id: gpg-import
        uses: coatl-dev/actions/gpg-import@v3.5.3
        with:
...
```yml
      - name: Create Pull Request
        uses: coatl-dev/actions/pr-create@v3.5.3
        with:
...
```yml
    - name: Set up Jython
      uses: coatl-dev/actions/setup-jython@v3.5.3
      with:
  ...
  pip-compile:
    ...
    steps:
      ...
      - name: pip-compile-312
        uses: coatl-dev/actions/uv-pip-compile@v3.5.3
        with:
          path: "${{ env.REQUIREMENTS_PATH }}"
          python-version: '3.12'
